  ...
  if (!$errors) {
    ...
    if (mysqli_query($dbc, $insert_into_customers)) {
      ...
      if (mysqli_query($dbc, $insert_into_orders)) {
        ...
        header("Location: http://localhost/onlineMobileStore/NidhiParekh_Project1/success.php");
      }
    }
  }
  mysqli_close($dbc);
...
<body>
  ...
  <div class="container">
    ...
    <form action="checkout.php" method="post" class="needs-validation row g-3" novalidate="">
        ...
        <ul class="list-group mb-3">
          <li class="list-group-item d-flex justify-content-between lh-sm">
            <div>
              <?php
                if(! empty($_SESSION["mobilename"])){
                  echo "<h6 class='my-0'>". $_SESSION["mobilename"]."</h6>";
                  echo "<small class='text-muted'>By " . $mobilebrand."</small>";
                }    
                if (empty($_SESSION["mobilename"])) {
                echo "<small class='text-muted'>Please Select Item</small>";
              }
              ?>
            </div>
            <?php
              if (! empty($_SESSION["mobilename"])) {
                echo "<span class='text-muted' id='price'>".$_SESSION["price"]."</span>";
              }
            ?>
          ...
          <li class="list-group-item d-flex justify-content-between">
            ...
            <?php
              if (! empty($_SESSION["mobilename"])) {
                echo "<strong id='total'>$".$_SESSION["price"]."</strong>";
              }
            ?>
      ...
      <form>
        <div class="row g-3">
          ...
          <div class="col-12">
            ...
            <input type="text" class="form-control" name="street" id="address2" placeholder="Street & unit" required="">
          </div>
        ...
        <?php
        if (empty($_SESSION["mobilename"])) {
          echo "<button class='w-100 btn btn-secondary btn-lg' type='submit' disabled>Place Order</button>";
        } else {
          echo "<button class='w-100 btn btn-primary btn-lg' type='submit'>Place Order</button>";
        }
        ?>
